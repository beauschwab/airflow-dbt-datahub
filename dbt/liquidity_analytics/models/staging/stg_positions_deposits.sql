...
/*
  stg_positions_deposits.sql
  --------------------------
  Staging model: dedup, type-cast, and partition raw deposit positions
  from the Core Banking System (CBS).
...
with source as (
    ...
    from {{ source('raw', 'positions_deposits') }}
    {% if is_incremental() %}
